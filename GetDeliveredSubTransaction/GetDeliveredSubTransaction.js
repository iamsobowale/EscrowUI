const fetchTableAsync = async () => {
    ...
    console.log(refId);
    const fet = await fetch("https://localhost:5001/api/TransactionType/GetDeliveredSubTransaction/" + refId);
...
const showTableAsync = async () => {
    ...
    tableBody.transaction.forEach(element => {
        ...
        <tr>
            ...
            <td><button type='submit' class="btn" id= ${element.reference} style="background-color:green">Accept</button></td>
            <td><button type='submit' class="btn2" id= ${element.reference} style="background-color:green">Reject</button></td>
        </tr>
     `
     test();
     test2();

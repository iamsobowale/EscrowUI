        ...
        data.transactionList.forEach(element => {
            ...
            if(element.numberOfSub ==0){
                ...
                <tr>
                    ...
                    <td><button type='button' class="btn" id =${element.reference_id} style="background-color:green"><strong>Deliver</strong></button></td>
                </tr>
              `
              completeTransaction ()
            }
...
function completeTransaction (){
    ...
    const get2 = document.querySelectorAll('.btn');
    get2.forEach(d =>  {
        d.addEventListener('click', (e) =>{
            console.log("dffffff")
            console.log(e.target.id)
            const fet = fetch("https://localhost:5001/api/Transaction/DoneTransaction?reference=" + e.target.id, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": "Bearer " + localStorage.getItem("token")

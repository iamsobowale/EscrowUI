...
const showTableAsync = async () => {
    ...
    tableBody.transactionList.forEach(element => {
        ...
        <tr>
            ...
            <td><button type='button' class="btn2" id = ${element.reference_id} style="background-color:green">View</button></td>
            <td><button type='button' class="btn" id = ${element.reference_id} style="background-color:green">Agree</button></td>
            <td><button type='button' class="btn" id = ${element.reference_id} style="background-color:green">Reject</button></td>
        </tr>
      `
      addbtn()
      test()
...
const test = () => {
    ...
    buttons.forEach(btn=> {
        btn.addEventListener('click', (e)=>{ 
            window.location.href = `/GetAllSubTranscationByTransactionNumber/getAllSubTransaction.html?id=${e.target.id}`;
        })
    })
}
showTableAsync();
